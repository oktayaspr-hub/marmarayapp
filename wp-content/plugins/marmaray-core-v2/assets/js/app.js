...
const renderStationCards = (idx) => {
  ...
  const makeCard = (trains, dir, dest, cls) => {
    if (!trains.length) return `
      <div class="train-card ${cls}">
        <div class="card-header"><span>${dir}</span><span class="dest-tag">→ ${dest}</span></div>
        <div class="card-body"><p class="no-train">Bu yönde sefer bulunamadı.</p></div>
      </div>`;
    const n = trains[0];
    return `
      <div class="train-card ${cls}">
        <div class="card-header"><span>${dir}</span><span class="dest-tag">→ ${dest}</span></div>
        <div class="card-body">
          <div class="next-train-row">
            <div class="big-mins" id="${cls}-mins">${n.remainingMin}</div>
            <div class="mins-label">dakika<br>sonra</div>
            <div class="exact-time">${n.timeStr}</div>
          </div>
          <div class="upcoming-list">
            ${trains.slice(1,5).map(t=>`
              <div class="upcoming-item">
                <span class="up-dest">${t.destination}</span>
                <span class="up-mins">${t.remainingMin} dk</span>
                <span class="up-time">${t.timeStr}</span>
              </div>`).join('')}
          </div>
        </div>
      </div>`;
  ...
  el.innerHTML = `
    <div class="station-cards-header">
      <h3 class="selected-station-name">${name} İstasyonu</h3>
      <div class="live-badge-pill">
        <span class="live-dot-anim"></span>
        <span id="live-badge-text" class="live-badge-label">CANLI</span>
      </div>
    </div>
    <div class="cards-row">
      ${makeCard(g2h,'Halkalı Yönü','Halkalı','card-blue')}
      ${makeCard(h2g,'Gebze Yönü','Gebze','card-red')}
    </div>`;
